...
Default guardrails are `--max-percent 90`, `--resume-percent 82`, `--max-ram-percent 90`, and `--max-gpu-mem-percent 90`.
...
Use `--batch` or `--workers` to override auto choices only when the machine is idle. Use `--plots` only when plots are needed; leaving it off is safer for RAM.
