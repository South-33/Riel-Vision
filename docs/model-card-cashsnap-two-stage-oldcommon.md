...
The deployable browser path should be checked with `scripts/smoke_browser_demo_cdp.cjs`, because Edge ONNX Runtime Web/canvas preprocessing is the deployment truth and can differ from PyTorch-generated proposal CSVs around borderline detector scores. Current Edge autorun smoke on the same shop-overlap image predicts `6` bills with `KHR 76,000`, `USD 0`, and classes `KHR_1000:1;KHR_10000:1;KHR_20000:3;KHR_5000:1`. Evaluating the browser-exported CSV against the draft labels gives `6/6` any-class matches and `4/6` same-class matches after the custom detector resizer.
