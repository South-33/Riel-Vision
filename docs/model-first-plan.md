...
Two-stage fusion refresh on the same date confirms the Python diagnostic setting: `det_threshold=0.17` or `0.175` with NMS IoU `0.75-0.90` gives 6 predictions for 6 draft notes, 6/6 any-class coverage, and 5/6 same-class recall. The Edge ONNX browser smoke is lower after preprocessing parity work: 6 predictions, 6/6 any-class, 4/6 same-class, KHR 76,000 vs the draft KHR 70,000 total, and final confusions `KHR_10000->KHR_1000` plus `KHR_5000->KHR_20000`. Treat both as calibration evidence from one draft-labeled image, not production evidence; add future labeled cases to `manifests/browser_smoke_cases.csv` and run `scripts/run_browser_smoke_cases.py`.
