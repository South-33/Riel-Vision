...
Two-stage fusion refresh on the same date confirms the diagnostic browser-stack setting: `det_threshold=0.17` or `0.175` with NMS IoU `0.75-0.90` gives 6 predictions for 6 draft notes, 6/6 any-class coverage, and 5/6 same-class recall. This is the current best deployable diagnostic behavior, still limited by one draft-labeled image and not production evidence.
