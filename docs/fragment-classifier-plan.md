...
- Current focused old/common classifier on the Roboflow overlap subset (`data/fragment_classifier_roboflow_partial_khr_oldcommon_eval_v1`) reached only `0.615` val and `0.593` test accuracy. `KHR_10000` is strong (`40/40` val, `26/30` test), but `KHR_20000` and `KHR_5000` mostly collapse into `KHR_10000`; target reviewed real partial/back crops for those confusions before replacing the browser classifier.
