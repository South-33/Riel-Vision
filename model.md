...
- `compare_yolo_metric_replicates.py` turns repeated model probes into promotion gates by checking candidate mean, worst seed, and worst per-class seed against a baseline; use it before calling any synthetic dose/curriculum change stable.
...
| 2026-06-05 | evaluation | keep | `compare_yolo_metric_replicates.py` adds a multi-run gate for model-side synthetic promotion. On the two dose2 seeds it rejects the candidate (`mean 0.832275`, `-0.003586` vs p24; worst seed `0.826076`, `-0.009785`; worst per-class `KHR_50000 -0.158238`, failures `KHR_2000`/`KHR_50000`), proving the single passing seed was not stable evidence. |
...
| 2026-06-05 | evaluation | reject | Stack rare-support dose2 does not fix seed fragility. Seed0 passes (`0.837617`, `+0.001756` vs p24, worst `KHR_50000 -0.049608` just inside the per-class guard), but seed1 rejects (`0.826476`, `-0.009385`, worst `KHR_50000 -0.156727`, two per-class failures); the replicate gate rejects the pair (`mean 0.832046`, `-0.003815`). The issue is not only hard fan fragmentation. |
