...
- Recipe diagnostic gates now support a `domain_gap` gate that composes a bounded real-control train list with a packaged WebGL root and runs `audit_yolo_domain_gap.py`; `webgl_real_scale_stack_geometry_v1` uses it to prove closeup-framed stack renders pass aggregate visible-note geometry before any model probe. `webgl_real_scale_stack_pose_v1` adds the first class-conditioned stack pose/exposure policy and a coarse per-class aspect guard; it improves the original giant aspect failures but is still diagnostic because strict per-class geometry does not pass.
