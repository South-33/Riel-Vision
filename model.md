...
Label policy: exact visible masks/ID colors are authoritative. Detect AABBs are compatibility labels for detect-only probes; OBB labels are useful only when the visible mask is compact enough that a rotated rectangle does not cover large hidden regions. If one physical bill is visible as disconnected islands, keep it one counted instance in mask metadata, but skip/flag the OBB label or route the fragments to a fragment/segmentation path.
...
- `scripts/render_webgl_variant_batch.py` renders/checks deterministic WebGL variants, writes a contact sheet, packages YOLO detect and OBB dataset views, and runs `check_yolo_dataset.py` by default. Smoke command: `rl python scripts\render_webgl_variant_batch.py --out-root data\synthetic\cashsnap_webgl_variant_batch_smoke --count 4`; variants 0-3 passed and wrote `data/synthetic/cashsnap_webgl_variant_batch_smoke/contact_sheet.png`.
...
- OBB export now audits connected components and rectangle fill before writing labels. On `cashsnap_webgl_fullschema_fan_batch_p0`, 34/67 visible instances exported OBB labels; 25 were skipped as loose min-area rectangles and 8 as fragmented visible masks. This is intentional: those instances remain in detect labels and ID masks, but should not train an OBB model on hidden-space boxes.
- Three-bill label QA set: `data/synthetic/cashsnap_webgl_3bill_label_qa_v0/` was packaged from the fixed stack render without relaunching WebGL while RAM was high. It has 3 detect boxes; OBB exported `KHR_10000` and `KHR_20000` while skipping `KHR_5000` because the visible mask had low rectangle fill (`0.2761`). Preview files: `variant_0000_detect_preview.jpg` and `variant_0000_obb_preview.jpg`.
