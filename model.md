...
| 2026-05-31 22:41 | renderer | keep | Kept Edge as the default WebGL browser because it is installed and GPU-backed, but added `--browser-executable`/`CASHSNAP_WEBGL_BROWSER` override plumbing through render, recipe, suite, and pipeline runners so Chrome/Chromium can be benchmarked without changing renderer code. |
